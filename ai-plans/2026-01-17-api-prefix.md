...
- Related docs:
  ...
  - `docs/system/Lifecycle.md` (mentions example `GET /health`)
  - `docs/Overview.md` (lists `GET /health`)
  - `docs/Architecture.md` (API flow example `GET /health`)
...
## Technical Approach
- Keep route modules (`src/app/api/routes/*.ts`) unchanged (they define paths like `/health`, `/digests`, `/news-items/...`).
...
## Open Questions
- Should we **hard break** legacy root routes (`/health`, `/digests`, `/news-items/...`) immediately (recommended per request), or keep them temporarily as deprecated aliases?
